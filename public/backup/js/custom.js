...
$("body").on("click", "#send_booking_confirmation_mail", function ()
{
	console.log("send_booking_confirmation_mail");
	var booking_id = $(this).data("id");
   $.get(APP_URL + "/booking-confirmation-email/" + booking_id, function (resonse)
	{
		
		resonse.success ? $.toast(
			{
				heading: "Success",
...
console.log("resonse",resonse.success);
...
$(function() {
	...
	$("body").on("click", "#add_supplier_service", function ()
	{
		console.log("Add supplier service Manual form"), $(".supplier_services_add_form").validate().resetForm(), $(".supplier_services_add_form").trigger("reset"),
		// $("#status_1").attr('checked', 'checked');
		// $("#status_2").attr('checked', false);
		// $("#status_3").attr('checked', false);
		$("#add_supplier_services_model").modal("show")
	});

	$("body").on("click", "#invoice_download", function ()
	{
		var booking_id = $(this).data("id");
		i = APP_URL + "/invoice-download/" + booking_id;
		$.ajax(
		{
			type: "POST",
			url: i,
