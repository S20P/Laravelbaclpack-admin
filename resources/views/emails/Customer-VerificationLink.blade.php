<meta charset="UTF-8">
<title>Title</title>

<style type="text/css">
    html {
        font: 14px/1 'Open Sans', sans-serif;
        line-height: normal;
    }

    body {
        margin: 0;
        padding: 0;
        color: #153643;
    }

    a {
        color: #153643;
    }

    strong {
        font-size: 14px;
    }
</style>



<table border="0" cellpadding="0" cellspacing="0" width="100%">
    <tbody>
        <tr>
            <td>
                <table align="center" border="0" cellpadding="0" cellspacing="0" width="600">
                    <tbody>
                        <tr>
                            <td bgcolor="#ffffff">
                                <table border="0" cellpadding="0" cellspacing="0" width="100%">
                                    <tbody>
                                        <tr>
                                            <td bgcolor="#eee" style="padding: 10px 20px 10px 20px;">
                                                <table border="0" cellpadding="0" cellspacing="0" width="100%">
                                                    <tbody>
                                                        <tr>
                                                            <td width="260" valign="middle">
                                                                <table border="0" cellpadding="0" cellspacing="0"
                                                                    width="100%">
                                                                    <tbody>
                                                                        <tr>
                                                                            <td>
                                                                                <a href="{{base_url}}"><img
                                                                                        src="{{base_url}}/images/logoMailSend.png"
                                                                                        alt="Logo" width="217px"
                                                                                        style="display: block;"></a>

                                                                            </td>
                                                                        </tr>
                                                                    </tbody>
                                                                </table>
                                                            </td>
                                                            <td style="font-size: 0; line-height: 0;" width="20">
                                                                &nbsp;
                                                            </td>
                                                            <td width="260" valign="middle">
                                                                <table border="0" cellpadding="0" cellspacing="0"
                                                                    width="100%">
                                                                    <tbody>
                                                                        <tr>
                                                                            <td>
                                                                                <h3 class="collapse"
                                                                                    style="text-align: right; font-weight: normal;">
                                                                                </h3>
                                                                            </td>
                                                                        </tr>

                                                                    </tbody>
                                                                </table>
                                                            </td>
                                                        </tr>
                                                    </tbody>
                                                </table>
                                            </td>
                                        </tr>
                                        <tr>
                                            <td style="padding: 10px 20px 10px 20px; color: #153643;">
                                                <!-- content body -->
                                                <table cellpadding="0" cellspacing="0" border="0"
                                                    style="border-collapse: collapse;" width="100%">
                                                    <tbody>
                                                        <tr style="width: 100%">
                                                            <td>
                                                                <table>
                                                                    <tbody>
                                                                    <tr>
                                                                    <td><br>
                                                                        Dear {{name}},
                                                                        <br>
                                                                    </td>
                                                                </tr>
                                                                <tr>
                                                                    <td>
                                                                        <br>
                                                                        Welcome to The Party Perfect!
                                                                        <br>
                                                                        <br>
                                                                    </td>
                                                                </tr>
                                                                <tr>
                                                                    <td>
                                                                    Thank you for registering with Party Perfect. Please click on the below 
                                                                    verification link to activate the account.<br>
                                                                    </td>
                                                                </tr>
                                                                <tr>
                                                                    <td>
                                                                    <br>
                                                                    Verification Link: <a href="{{verificationLink}}">{{verificationLink}}</a>
                                                                  <br> <br>
                                                                    </td>
                                                                      </tr>
                                                                         <tr>
                                                                        </tr>
                                                                    </tbody>
                                                                </table>
                                                                <!-- content body end -->
                                                            </td>
                                                        </tr>
                                                        <tr>
                                                            <td bgcolor="#715DFC" style="padding: 30px 30px 30px 30px;">
                                                            </td>
                                                        </tr>
                                                        <tr>
                                                            <td>
                                                                <table style="width: 100%;">
                                                                    <tbody>
                                                                        <tr>
                                                                            <td align="center">
                                                                                <p style="font-size: 12px;">
                                                                                    <a
                                                                                        href="{{base_url}}/supplier-tack-and-care">SUPPLIER
                                                                                        T&amp;C</a>|
                                                                                    <a
                                                                                        href="{{base_url}}/help">HELP</a>|
                                                                                    <a href="{{base_url}}/faq">FAQ'S</a>
                                                                                </p>
                                                                            </td>
                                                                        </tr>
                                                                    </tbody>
                                                                </table>
                                                            </td>
                                                        </tr>
                                                    </tbody>
                                                </table>
                                            </td>
                                        </tr>
                                    </tbody>
                                </table>
                            </td>
                        </tr>
                    </tbody>
                </table>
            </td>
        </tr>
    </tbody>
</table>